using System;
using NT8.SDK;              // ISdk, IClock, SystemClock, IBacktestHooks, IOrders
using NT8.SDK.Risk;         // RiskMode, RiskOptions, RiskEngine
using NT8.SDK.Sizing;       // SizeEngine
using NT8.SDK.Session;      // CmeBlackoutService
using NT8.SDK.Trailing;     // TrailingEngine

#if INCLUDE_QA
using QA.TestKit;           // NoopBacktestHooks (only if you define INCLUDE_QA)
#endif
...
namespace NT8.SDK.Facade
{
    ...
    public sealed class SdkBuilder
    {
        ...
        public ISdk Build()
        {
            ...
            // Backtest hooks: use QA when available, otherwise pass null (SDK should handle null safely)
            IBacktestHooks backtest =
#if INCLUDE_QA
                new NoopBacktestHooks();
#else
                null;
#endif
